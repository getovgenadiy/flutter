...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'Flutter Datastore Demo Home Page'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _configureAmplify() async {
      ...
      // apiPlugin = AmplifyAPI(modelProvider: ModelProvider.instance);
...
void _addPost(int cnt) async {
  Post newPost = Post(
      title: 'Post number: $cnt being saved', rating: 15, status: PostStatus.DRAFT, content: 'Post No:${(1000+cnt)}');
...
void _updatePost() async{
  Post oldPost = (await Amplify.DataStore.query(Post.classType,
      where: Post.ID.eq('dccb479c-e27d-43dd-9960-9cfae7015dcf')))[0];
  Post newPost = oldPost.copyWith(id: oldPost.id, title: 'Updated Title');

  await Amplify.DataStore.save(newPost);
}

void _delepePost() async{
  (await Amplify.DataStore.query(Post.classType, where: Post.ID.eq('dccb479c-e27d-43dd-9960-9cfae7015dcf')))
